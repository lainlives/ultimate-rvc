...
from ultimate_rvc.rvc.lib.predictors.HPARMVPE import HPARMVPE0Predictor
...
class F0Extractor:
    ...
    def extract_f0(self):
        ...
        elif method == "hpa-rmvpe":
            model_hpa_rmvpe = HPARMVPE0Predictor(
                os.path.join(str(RVC_MODELS_DIR), "predictors", "hpa-rmvpe.pt"),
                device=config.device,
            )
            f0 = model_hpa_rmvpe.infer_from_audio(self.wav16k, thred=0.03)

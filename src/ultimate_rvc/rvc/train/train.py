...
def train_and_evaluate(
    ...
    if rank == 0:
        ...
        if epoch % save_every_epoch == 0:
            ...
            audio_tensor = o[0, :, :].float()

            # Clean NaN and Inf values caused by bf16 instability
            audio_tensor = torch.nan_to_num(
                audio_tensor, nan=0.0, posinf=1.0, neginf=-1.0
            )

            # Clamp to prevent cast overflow in TensorBoard
            audio_tensor = torch.clamp(audio_tensor, min=-1.0, max=1.0)

            audio_dict = {f"gen/audio_{global_step:07d}": audio_tensor}
            summarize(
                writer=writer,
                global_step=global_step,
                images=image_dict,
